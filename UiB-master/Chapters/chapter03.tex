...
\end{figure}
\justify
The STL process extract the most significant and relevant feature or information in the intrinsic mode function. The result is a signal with strong sinusoidal components. The signal obtained from the STL process is called pulse signal.
\begin{figure}[H] %  figure placement: here, top, bottom, or page
   \centering
   \includegraphics[width=6in]{../fig/emd-stl.png} 
   \caption{ A pulse signal extracted by applying EMD followed by STL. The pulses represents the periodic high frequency signal emitted by a bearing when the roller elements pass a crack located in the outer ring.}
   \label{fig:emd-stl}
\end{figure}
\justify
