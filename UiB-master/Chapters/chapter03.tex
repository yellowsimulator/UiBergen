...
\end{equation}
However, the Hilbert Huang transform will decompose the function as 
\begin{equation}\label{eq:hht1}
f(t) = \sum_{j=1}^{n}c_{j}(t) + r(t).
\end{equation}
In equation (\ref{eq:basis}) the basis $\varphi_{j}(t)$ are chosen before hand and the coefficients are computed through an integral or summation operation. On the hand, in equation (\ref{eq:hht1}) is one say clearly state that the basis function are directely computed based on the function $f(t)$. This is illustrates the concept of adaptivity, central to the Hilbert Huang transform.
