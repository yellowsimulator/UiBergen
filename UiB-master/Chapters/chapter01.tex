...
\justify
One of the issue of the Hilbert Huang transform is the selection of the appropriate intrinsic mode function for analysis.
(\cite{osman2013a}, \cite{osman2013b} ) uses a similarity measure to select the target IMF and uses the energy spectrum to identify bearing failure frequencies. Wheres (\cite{osman2014}) uses a weighted D'Agostino Pearson (DP) normality test to select the more relevant IMFs (IMF representing defect component). The DP uses both the statistical skewness and the the kurtosis to assess normality.
