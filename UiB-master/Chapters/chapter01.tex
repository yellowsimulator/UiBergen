...
I this thesis two methods are proposed for bearing fault detection: The first method uses the Hilbert Huang transform, coupled with a robust seasonal trend decomposition method, to generate a signal, that contains all the diagnosis information, related to bearing faults. The resulting signal is a periodic high frequency pulse like signal. By extracted its frequency components, one is able to identify bearing failure frequencies.
\justify
The second method uses wavelet transform to generate two addition signals from a target bearing vibration signal. 
To quantify bearing health, the interquartile range (IQR), which is a robust statistical measure of dispersion, is used.
By computing the IQR of the two additional signals, for a multiple bearing vibration signals, a set of points are generated in the two dimensional plan. Furthermore, by applying the one class support vector machine, one can classify bearings into two classes: healthy and unhealthy. In addition, a bearing with failure follows a trajectory which is terms bearing failure trajectory.
%
