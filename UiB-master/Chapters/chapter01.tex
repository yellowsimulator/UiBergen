...
The latter question has been the driving force, behind the adoption of what is commonly called predictive maintenance.
...
\end{figure}
\justify
Because a bearing vibration signal can be \say{contaminated} by other signals from different sources, most bearing fault detection methods apply a set of transformations to the target signal, before applying the Fourier transform. These \say{pre-processing} aim at isolating part of the signal that contained relevant diagnostic information. One of the most widely used method which accomplish this with elegance, is the so call high frequency resonance technique (HFRT). Its apply a series of \say{filtering} operations in order to cipher only part of the vibration signal, that contained potential traces of failure. The filtering operations, which are mathematical operations, are high, low and band pass filtering. As their name indicate, a high pass filtering operation remove all low frequencies in a target signal, while a low pass filter accomplish the apposite.
A band pass filter remove all frequencies bellow and above predefined thresholds. 
\justify
As we will see later, a bearing with a defect generate pulse like signal, buried deep inside the vibration signal, which can render the signal non linear and non stationary. Such pulses are temporal events, that the HFRT attempts to isolate though filtering operations. Although efficient, the spectrum derived from the HFRT can be sometime noisy, rendering failure frequency detection challenging. This is because, the filtering operations are not designed to isolate temporal events. To mitigate this issue, this thesis presents an alternative method, based on Hilbert Huang transform (HHT) for bearing fault detection. By design, the Hilbert Huang transform extracts temporal events from a signal.
